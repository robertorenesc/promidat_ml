...
class ACP(Exploratorio):
    ...
    def __init__(self, datos, n_componentes = 5):
        ...
        self.__modelo = PCA(n_components = n_componentes).fit(self.datos)
    ...
    def plot_plano_principal(self, ejes = [0, 1], ind_labels = True, titulo = 'Plano Principal', groups = []):
        colors = ["blue", "red", "green", "yellow", "magenta"]
        coordenadas_colores = []
        for i in range(len(groups)):
            coordenadas_colores.append(colors[groups[i]])
        x = self.coordenadas_ind[ejes[0]].values
        y = self.coordenadas_ind[ejes[1]].values
        plt.subplots(figsize=(13, 13))
        plt.style.use('seaborn-whitegrid')
        for i in range(len(x)):
            #print(f"({x[i]}, {y[i]}, {coordenadas_colores[i]})")
            plt.plot(x[i], y[i], "o", color = coordenadas_colores[i])
        plt.title(titulo)
    ...
    def analisis(self, columnas = [], columna_normalidad = ""):
        super.analisis(columnas = columnas, columna_normalidad = columna_normalidad)
        self.plot_plano_principal()
        self.plot_circulo()
